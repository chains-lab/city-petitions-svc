...
func (s Service) SignPetition(ctx context.Context, req *svc.SignPetitionRequest) (*svc.Signature, error) {
	...
	sign, err := s.app.SignPetition(ctx, initiator.ID, petitionId)
	if err != nil {
		logger.Log(ctx).Errorf("failed to sign petition: %v", err)

		return nil, err
	}

	logger.Log(ctx).Infof("initiator %s is signed petition %s", initiator.ID, petitionId)

	return responses.Signature(sign), nil
}
